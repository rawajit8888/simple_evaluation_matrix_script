...
class MultiTaskBertModel:
    ...
    def _extract_taxonomy_from_task(self, task):
        ...
        # Extract subject and message fields from task data
        if 'data' in task:
            d = task['data']
            ...
            for key in ['message', 'Message', 'body', 'email_body', 'mail_body', 'email_message']:
                if key in d and d[key]:
                    ...
                    break
            # If no separate message field found, fall back to text
    ...
    def fit(self, event, data, tasks, **kwargs):
        ...
        # Cap Feedback > No Reply > Ignore at 500 samples
        # Without cap: 2236 samples = 16.7% of data causes model to be biased toward Feedback
        FEEDBACK_CLASS = 'Feedback > No Reply > Ignore'
        FEEDBACK_CAP   = 500
        feedback_rows  = df['querytype'] == FEEDBACK_CLASS
        if feedback_rows.sum() > FEEDBACK_CAP:
            original_count = int(feedback_rows.sum())
            keep_feedback  = df[feedback_rows].sample(n=FEEDBACK_CAP, random_state=42)
            df             = pd.concat([df[~feedback_rows], keep_feedback], ignore_index=True)
            self.logger.info(
                f"✂️  Feedback cap applied: {original_count} -> {FEEDBACK_CAP} samples "
                f"(removed {original_count - FEEDBACK_CAP} to reduce prediction bias)"
            )
        else:
            self.logger.info(f"Feedback class has {feedback_rows.sum()} samples — no cap needed")
        ...
        if len(qt_df) > 0:
            ...
            # Save test_split.csv — used later to run inference and compare predictions
            try:
                test_split_path = os.path.join(self.model_dir, 'test_split.csv')
                save_cols = [c for c in ['subject', 'message', 'text', 'masterdepartment', 'department', 'querytype']
                             if c in qt_test_df.columns]
                qt_test_df[save_cols].to_csv(test_split_path, index=False, encoding='utf-8')
                self.logger.info(f"💾 test_split.csv saved → {test_split_path} ({len(qt_test_df)} rows)")
            except Exception as e:
    ...
    def _train_level1(self, train_df, test_df):
        ...
        # Class weights — sqrt-softened so rare classes get more attention without overfitting
        # Old: clip(0.1, 10.0) — too aggressive, model memorised rare classes
        # New: sqrt then clip(0.5, 5.0) — gentler, model learns patterns not just memorises
        ...
        self.logger.info(f"⚖️  L1 class weights (sqrt-softened) — min: {raw_weights.min():.3f}  max: {raw_weights.max():.3f}  "
                         f"classes at 5x cap: {(raw_weights >= 4.99).sum()}")
    ...
    def _train_level2(self, train_df, test_df):
        ...
        # Class weights — sqrt-softened so rare departments get attention without overfitting
        raw_weights = compute_class_weight(
            class_weight='balanced',
        ...
        raw_weights  = np.clip(raw_weights, 0.5, 5.0)
        class_weights_tensor = torch.tensor(raw_weights, dtype=torch.float32).to(device)
        self.logger.info(f"⚖️  L2 class weights (sqrt-softened) — min: {raw_weights.min():.3f}  max: {raw_weights.max():.3f}  "
                         f"classes at 5x cap: {(raw_weights >= 4.99).sum()}")
        ...
        self.logger.info("            (same as how it was trained — teacher forcing applies to eval too)")
    ...
    def _train_level3(self, train_df, test_df):
        ...
        # Prepare conditional input for TRAINING
        # Training: use ground truth prefixes (as before)
        train_texts_conditional = [
            f"MasterDepartment: {row['masterdepartment']} Department: {row['department']} Email: {row['text']}"
            for _, row in train_df.iterrows()
        ]

        # Prepare conditional input for TEST EVALUATION
        # FIX: use L1 and L2 model predictions as prefix — NOT ground truth
        # This removes teacher forcing from test evaluation so the score is honest
        self.logger.info("🔧 Building test prefixes using L1+L2 MODEL PREDICTIONS (not ground truth)...")
        l1_model_path = os.path.join(self.model_dir, self.finetuned_model_name)
        l2_model_path = os.path.join(self.model_dir, 'department_model')

        test_texts_conditional = []
        for _, row in test_df.iterrows():
            # Default fallback to ground truth if models not loadable
            pred_masterdept = row['masterdepartment']
            pred_dept       = row['department']
            try:
                # Run L1 prediction
                l1_m = MasterDepartmentModel.load(l1_model_path)
                l1_m.to(device)
                l1_m.eval()
                enc_ids, enc_mask = self._tokenize_and_pad([row['text']], self.tokenizer, 256)
                with torch.no_grad():
                    logits = l1_m(enc_ids.to(device), enc_mask.to(device))
                pred_masterdept = l1_m.encoder.inverse_transform(
                    [torch.argmax(logits, dim=1).cpu().numpy()[0]]
                )[0]
                # Run L2 prediction with L1 predicted prefix
                l2_input = f"MasterDepartment: {pred_masterdept} Email: {row['text']}"
                l2_m = DepartmentNNModel.load(l2_model_path)
                l2_m.to(device)
                l2_m.eval()
                enc_ids2, enc_mask2 = self._tokenize_and_pad([l2_input], self.tokenizer, 256)
                with torch.no_grad():
                    logits2 = l2_m(enc_ids2.to(device), enc_mask2.to(device))
                pred_dept = l2_m.encoder.inverse_transform(
                    [torch.argmax(logits2, dim=1).cpu().numpy()[0]]
                )[0]
            except Exception as e:
                self.logger.warning(f"L1/L2 prediction failed for test row, using GT prefix: {e}")
            test_texts_conditional.append(
                f"MasterDepartment: {pred_masterdept} Department: {pred_dept} Email: {row['text']}"
            )
        self.logger.info(f"✅ Test prefixes built using model predictions ({len(test_texts_conditional)} rows)")
        ...
        # Class weights — sqrt-softened so rare querytypes get attention without overfitting
        # 148 classes with 62 having <10 samples — weights help but old clip(0.1,10) was too harsh
        raw_weights = compute_class_weight(
            class_weight='balanced',
        ...
        raw_weights  = np.clip(raw_weights, 0.5, 5.0)
        class_weights_tensor = torch.tensor(raw_weights, dtype=torch.float32).to(device)
        self.logger.info(f"⚖️  L3 class weights (sqrt-softened) — min: {raw_weights.min():.3f}  max: {raw_weights.max():.3f}  "
                         f"classes at 5x cap: {(raw_weights >= 4.99).sum()}")
        ...
        self.logger.info("            (teacher forcing — same as training, NOT chained model outputs)")
        ...
        # Save predictions_on_test_split.csv
        # One row per test email with subject, message, ground truth and predicted labels
        try:
            pred_csv_path = os.path.join(self.model_dir, 'predictions_on_test_split.csv')
            test_df_reset  = test_df.reset_index(drop=True)
            pred_rows = []
            for idx, (_, row) in enumerate(test_df_reset.iterrows()):
                pred_rows.append({
                    'subject'              : row.get('subject', ''),
                    'message'              : row.get('message', row.get('text', '')),
                    'text'                 : row.get('text', ''),
                    'true_masterdepartment': row.get('masterdepartment', ''),
                    'true_department'      : row.get('department', ''),
                    'true_querytype'       : decoded_qt_true[idx],
                    'predicted_querytype'  : decoded_qt_pred[idx],
                    'correct'              : 'YES' if decoded_qt_true[idx] == decoded_qt_pred[idx] else 'NO',
                })
            pred_df = pd.DataFrame(pred_rows)
            pred_df.to_csv(pred_csv_path, index=False, encoding='utf-8')
            correct_count = (pred_df['correct'] == 'YES').sum()
            self.logger.info(
                f"💾 predictions_on_test_split.csv saved → {pred_csv_path} "
                f"({len(pred_df)} rows | {correct_count} correct | {correct_count/len(pred_df)*100:.1f}% accuracy)"
            )
        except Exception as e:
