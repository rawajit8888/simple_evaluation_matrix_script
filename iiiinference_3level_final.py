...
# CONFIG
# =========================
PREDICT_URL_BERT = "http://localhost:9093/predicttext"   # <-- update if needed
...
TRUE_MASTERDEPT_COL = "masterdepartment_true"
TRUE_DEPT_COL       = "department_true"
TRUE_QUERYTYPE_COL  = "querytype_true"
...
def build_payload(text):
    return {
        "texts": [
            {
                "id": "3",
                "text": text          # no json.dumps — text is already a string
            }
        ]
    }
...
def parse_3level_response(result):
    ...
    try:
        ...
        for item in api_block:
            ...
            score         = item.get("value", {}).get("score", "")
...
# SMOKE TEST
# =========================
print(f"\n🧪 Smoke test — verifying endpoint is reachable...")
try:
    _test_resp = requests.post(
        PREDICT_URL_BERT,
        headers=headers,
        json=build_payload(str(df.iloc[0][EMAIL_COL])),
        proxies=PROXIES,
        timeout=30
    )
    if _test_resp.status_code != 200:
        print(f"❌ Smoke test failed — HTTP {_test_resp.status_code}. Check PREDICT_URL_BERT.")
        exit(1)
    print("✓ Smoke test passed")
except Exception as e:
    print(f"❌ Could not reach {PREDICT_URL_BERT}: {e}")
    exit(1)
...
for idx, row in tqdm(df.iterrows(), total=len(df)):
    ...
    try:
        ...
        parsed = parse_3level_response(response.json())
...
if TRUE_MASTERDEPT_COL in df.columns and TRUE_DEPT_COL in df.columns and TRUE_QUERYTYPE_COL in df.columns:
    ...
    if len(eval_df) == 0:
        print("❌ No valid predictions — check that the server is returning responses correctly.")
        exit(1)
    ...
    summary_df = pd.DataFrame({
        'Level':      ['Level 1 (MasterDept)', 'Level 2 (Department)', 'Level 3 (QueryType)'],
    ...
    with open("metrics_3level.txt", "w", encoding="utf-8") as f:
        ...
        for label, y_t, y_p, acc, f1 in [
            ("LEVEL 1: MASTERDEPARTMENT", y_true_l1, y_pred_l1, acc_l1, f1_l1),
            ("LEVEL 2: DEPARTMENT",       y_true_l2, y_pred_l2, acc_l2, f1_l2),
            ("LEVEL 3: QUERYTYPE",        y_true_l3, y_pred_l3, acc_l3, f1_l3),
        ]:
            f.write(label + "\n")
            f.write("-" * 80 + "\n")
    ...
    axes[0].set_title(f'Level 1: MasterDept\nAccuracy: {acc_l1:.2%}')
    ...
    print("💾 Saved → confusion_matrices_3level.png")
    plt.show()
    ...
    bars = ax.bar(bar_labels, accuracies, color=colors, alpha=0.8, edgecolor='black', linewidth=1.5)
    ...
    print("💾 Saved → accuracy_comparison_3level.png")
    plt.show()

else:
    print("\n⚠️  Ground truth columns not found — skipping evaluation.")
    print(f"   Expected: {TRUE_MASTERDEPT_COL}, {TRUE_DEPT_COL}, {TRUE_QUERYTYPE_COL}")
