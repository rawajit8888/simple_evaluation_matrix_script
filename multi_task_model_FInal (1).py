...
class MultiTaskBertModel:
    ...
    def fit(self, event, data, tasks, **kwargs):
        ...
        if len(qt_df) > 0:
            ...
            # Save test_split.csv immediately after train/test split
            try:
                test_split_path = os.path.join(self.model_dir, 'test_split.csv')
                save_cols = [c for c in ['subject', 'message', 'text',
                                         'masterdepartment', 'department', 'querytype']
                             if c in qt_test_df.columns]
                qt_test_df[save_cols].to_csv(test_split_path, index=False, encoding='utf-8')
                self.logger.info("=" * 70)
                self.logger.info("💾 TEST SPLIT CSV SAVED")
                self.logger.info(f"   Path    : {test_split_path}")
                self.logger.info(f"   Rows    : {len(qt_test_df)}")
                self.logger.info(f"   Columns : {save_cols}")
                self.logger.info("=" * 70)
            except Exception as e:
                self.logger.warning(f"Could not save test_split.csv: {e}")
    ...
    def _train_level3(self, train_df, test_df):
        ...
        # Save predictions_on_test_split.csv with subject/message/GT/predicted
        try:
            _pcsv = os.path.join(self.model_dir, 'predictions_on_test_split.csv')
            ...
            self.logger.info("=" * 70)
            self.logger.info("💾 PREDICTIONS ON TEST SPLIT CSV SAVED")
            ...
            self.logger.warning(f"Could not save predictions_on_test_split.csv: {e}")
