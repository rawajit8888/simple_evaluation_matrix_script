...
class MultiTaskBertModel:
    ...
    def _extract_taxonomy_from_task(self, task):
        ...
        # Extract subject and message from task data (saved to test_split.csv)
        if 'data' in task:
            d = task['data']
            for key in ['subject', 'Subject', 'email_subject', 'mail_subject']:
                if key in d and d[key]:
                    result['subject'] = str(d[key])
                    self.logger.debug(f"Extracted subject from field '{key}'")
                    break
            for key in ['message', 'Message', 'body', 'email_body', 'mail_body', 'email_message']:
                if key in d and d[key]:
                    result['message'] = str(d[key])
                    self.logger.debug(f"Extracted message from field '{key}'")
                    break
    ...
    def _save_evaluation_folder(self, level_name, y_true, y_pred, train_count, test_count):
        ...
        Creates an evaluation folder with:
          ...
          - metrics_summary.txt        → accuracy, weighted-F1, macro-F1 at a glance
          - predictions.csv            → side-by-side true vs predicted for every test sample
        ...
        self.logger.info(f"📁 Evaluation folder: {eval_dir}")
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"{'='*70}\n\n")
            f.write(report_str)
        self.logger.info(f"💾 classification_report.txt saved")

        # ── 2. metrics_summary.txt ────────────────────────────────────────────
        ...
        # Per-class F1 — find worst 10 and best 10
        ...
        with open(summary_path, 'w', encoding='utf-8') as f:
            f.write(f"{'='*70}\n")
            f.write(f"  METRICS SUMMARY — {level_name.upper()}\n")
            f.write(f"  Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"{'='*70}\n\n")
            f.write(f"  Accuracy     : {accuracy:.4f}\n")
            f.write(f"  Macro-F1     : {macro_f1:.4f}   ← use THIS to pick best epoch\n")
            ...
            f.write(f"{'─'*70}\n")
            f.write(f"  WORST 10 CLASSES (lowest F1 — need more data or better labels)\n")
            f.write(f"{'─'*70}\n")
            ...
            for cls, score in reversed(best10):
                f.write(f"  {score:.4f}  {cls}\n")
        self.logger.info(f"💾 metrics_summary.txt saved")
        ...
        self.logger.info(f"💾 predictions.csv saved")

        self.logger.info(
            f"✅ Evaluation folder complete → {eval_dir}\n"
    ...
    def fit(self, event, data, tasks, **kwargs):
        ...
        # ── Save ALL training logs to a timestamped .txt file ─────────────────
        ...
        self.logger.info(f"📝 Training log saving to: {log_filename}")
        # ──────────────────────────────────────────────────────────────────────
        ...
        # Cap dominant Feedback class to prevent prediction bias
        FEEDBACK_CLASS = 'Feedback > No Reply > Ignore'
        FEEDBACK_CAP   = 500
        feedback_mask  = df['querytype'] == FEEDBACK_CLASS
        feedback_count = int(feedback_mask.sum())
        if feedback_count > FEEDBACK_CAP:
            keep_feedback = df[feedback_mask].sample(n=FEEDBACK_CAP, random_state=42)
            df            = pd.concat([df[~feedback_mask], keep_feedback], ignore_index=True)
            self.logger.info("=" * 70)
            self.logger.info("✂️  FEEDBACK CLASS CAP APPLIED")
            self.logger.info(f"   Class   : {FEEDBACK_CLASS}")
            self.logger.info(f"   Before  : {feedback_count} samples")
            self.logger.info(f"   After   : {FEEDBACK_CAP} samples")
            self.logger.info(f"   Removed : {feedback_count - FEEDBACK_CAP} (reduces prediction bias)")
            self.logger.info("=" * 70)
        else:
        ...
        if level1_model_exists:
            ...
            # ── Save test_split.csv — FULL data, ALL labels, BEFORE any training ─
            # This is the master reference file: every test email with all 3 GT labels
            try:
                full_split_path = os.path.join(self.model_dir, 'test_split.csv')
                full_save_cols  = [c for c in ['subject', 'message', 'text',
                                               'masterdepartment', 'department', 'querytype']
                                   if c in level1_test_df.columns]
                level1_test_df[full_save_cols].to_csv(full_split_path, index=False, encoding='utf-8')
                self.logger.info("=" * 70)
                self.logger.info("💾 test_split.csv SAVED — FULL TEST DATA, ALL LABELS (before any training)")
                self.logger.info(f"   Path    : {full_split_path}")
                self.logger.info(f"   Rows    : {len(level1_test_df)}")
                self.logger.info(f"   Columns : {full_save_cols}")
                self.logger.info("   ℹ️  This file is the ground-truth reference for all 3 levels")
                self.logger.info("=" * 70)
            except Exception as _e:
                self.logger.warning(f"⚠️  Could not save test_split.csv: {_e}")

            # ── Save level1_test_split.csv BEFORE training starts ─────────────
            try:
                l1_test_split_path = os.path.join(self.model_dir, 'level1_test_split.csv')
                l1_save_cols = [c for c in ['subject', 'message', 'text', 'masterdepartment']
                                if c in level1_test_df.columns]
                level1_test_df[l1_save_cols].to_csv(l1_test_split_path, index=False, encoding='utf-8')
                self.logger.info("=" * 70)
                self.logger.info("💾 LEVEL 1 TEST SPLIT SAVED (before training)")
                self.logger.info(f"   Path    : {l1_test_split_path}")
                self.logger.info(f"   Rows    : {len(level1_test_df)}")
                self.logger.info(f"   Columns : {l1_save_cols}")
                self.logger.info("=" * 70)
            except Exception as _e:
                self.logger.warning(f"⚠️  Could not save level1_test_split.csv: {_e}")
        ...
        if len(dept_df) > 0:
            ...
            # ── Save level2_test_split.csv BEFORE training starts ─────────────
            try:
                l2_test_split_path = os.path.join(self.model_dir, 'level2_test_split.csv')
                l2_save_cols = [c for c in ['subject', 'message', 'text',
                                             'masterdepartment', 'department']
                                if c in dept_test_df.columns]
                dept_test_df[l2_save_cols].to_csv(l2_test_split_path, index=False, encoding='utf-8')
                self.logger.info("=" * 70)
                self.logger.info("💾 LEVEL 2 TEST SPLIT SAVED (before training)")
                self.logger.info(f"   Path    : {l2_test_split_path}")
                self.logger.info(f"   Rows    : {len(dept_test_df)}")
                self.logger.info(f"   Columns : {l2_save_cols}")
                self.logger.info("=" * 70)
            except Exception as _e:
                self.logger.warning(f"⚠️  Could not save level2_test_split.csv: {_e}")
        ...
        if len(qt_df) > 0:
            ...
            # Save level3_test_split.csv BEFORE training starts
            try:
                l3_test_split_path = os.path.join(self.model_dir, 'level3_test_split.csv')
                l3_save_cols = [c for c in ['subject', 'message', 'text',
                                             'masterdepartment', 'department', 'querytype']
                                if c in qt_test_df.columns]
                qt_test_df[l3_save_cols].to_csv(l3_test_split_path, index=False, encoding='utf-8')
                self.logger.info("=" * 70)
                self.logger.info("💾 LEVEL 3 TEST SPLIT SAVED (before training)")
                self.logger.info(f"   Path    : {l3_test_split_path}")
                self.logger.info(f"   Rows    : {len(qt_test_df)}")
                self.logger.info(f"   Columns : {l3_save_cols}")
                self.logger.info("=" * 70)
            except Exception as _e:
                self.logger.warning(f"⚠️  Could not save level3_test_split.csv: {_e}")
    ...
    def _train_level1(self, train_df, test_df):
        ...
        # ═══════════════════════════════════════════════════════════════════
        # PROOF LOG — LEVEL 1 TRAINING INPUT
        # ═══════════════════════════════════════════════════════════════════
        self.logger.info("")
        self.logger.info("=" * 70)
        self.logger.info("📋 LEVEL 1 TRAINING — WHAT GOES INTO THE MODEL")
        self.logger.info("=" * 70)
        self.logger.info("  ✅ INPUT  : Raw email text ONLY — no prefix, no context")
        self.logger.info("  ✅ LABEL  : 'masterdepartment' column — read directly from Label Studio annotation")
        self.logger.info("  ✅ SOURCE : Human annotator labelled this, NOT any model output")
        self.logger.info(f"  ✅ LABEL COLUMN USED : df['masterdepartment']")
        self.logger.info(f"  ✅ SAVE PATH : {os.path.join(self.model_dir, self.finetuned_model_name)}")
        self.logger.info("")
        self.logger.info("  📌 SAMPLE INPUTS GOING INTO LEVEL 1 MODEL:")
        for idx, (_, row) in enumerate(train_df.head(3).iterrows()):
            self.logger.info(f"  ┌─ Sample {idx+1} ────────────────────────────────────────────────")
            self.logger.info(f"  │  INPUT TEXT  : \"{str(row['text'])}\"")
            self.logger.info(f"  │  LABEL (GT)  : \"{row['masterdepartment']}\"  ← from Label Studio annotation")
            self.logger.info(f"  └──────────────────────────────────────────────────────────────")
        self.logger.info("=" * 70)
        self.logger.info("")
        # ═══════════════════════════════════════════════════════════════════
        ...
        # ── Training setup ────────────────────────────────────────────────────
        NUM_EPOCHS    = int(os.getenv('L1_TRAIN_EPOCHS', 8))
        LEARNING_RATE = float(os.getenv('LEARNING_RATE', 2e-5))

        # Class weights sqrt-softened: clip(0.1,10) was too aggressive
        # New: sqrt then clip(0.5,5.0) — model learns patterns not memorises
        ...
        self.logger.info(f"⚖️  L1 class weights (sqrt-softened) — min: {raw_weights.min():.3f}  "
                         f"max: {raw_weights.max():.3f}  classes at 5x cap: {(raw_weights >= 4.99).sum()}")
        ...
        # ── Auto-select best epoch by Macro-F1 and copy to production folder ──
        best_epoch_num, best_macro, best_ckpt_dir = max(epoch_macro_scores, key=lambda x: x[1])
        self.logger.info("=" * 80)
        self.logger.info(f"🏆 L1 BEST EPOCH AUTO-SELECTED: Epoch {best_epoch_num} | Macro-F1: {best_macro:.4f}")
        self.logger.info(f"   Source : {best_ckpt_dir}")
        self.logger.info("   Epoch Summary:")
        for ep_num, ep_f1, _ in epoch_macro_scores:
            marker = " ← BEST" if ep_num == best_epoch_num else ""
        ...
        self.logger.info(f"✅ Best epoch {best_epoch_num} copied → {save_path}")
        self.logger.info("=" * 80)

        # ── Final evaluation using best epoch weights ──────────────────────────
        self.logger.info("📊 Evaluating Level 1 model (best epoch)...")
        ...
        # ── Log each test email flow for Level 1 ──────────────────────────────
        self.logger.info("")
        self.logger.info("=" * 70)
        self.logger.info("🧪 LEVEL 1 — TEST SET EVALUATION (20% split) — EMAIL BY EMAIL")
        ...
        self.logger.info(f"✅ MasterDepartment (best epoch {best_epoch_num}) - Accuracy: {masterdept_accuracy:.4f} | F1: {masterdept_f1:.4f}")
        ...
        # ── Save level1_predictions.csv ──────────────────────────────────────
        # Contains every test email with its ground truth and predicted MasterDepartment
        try:
            _l1_pred_csv = os.path.join(self.model_dir, 'level1_predictions.csv')
            _l1_tdf      = test_df.reset_index(drop=True)
            _l1_rows     = []
            for _li, _lr in _l1_tdf.iterrows():
                _l1_rows.append({
                    'subject'                   : _lr.get('subject', ''),
                    'message'                   : _lr.get('message', _lr.get('text', '')),
                    'text'                      : _lr.get('text', ''),
                    'true_masterdepartment'     : decoded_masterdept_true[_li],
                    'predicted_masterdepartment': decoded_masterdept_pred[_li],
                    'correct'                   : 'YES' if decoded_masterdept_true[_li] == decoded_masterdept_pred[_li] else 'NO',
                })
            _l1_pdf  = pd.DataFrame(_l1_rows)
            _l1_corr = (_l1_pdf['correct'] == 'YES').sum()
            _l1_pdf.to_csv(_l1_pred_csv, index=False, encoding='utf-8')
            self.logger.info("=" * 70)
            self.logger.info("💾 LEVEL 1 PREDICTIONS CSV SAVED (after training)")
            self.logger.info(f"   Path      : {_l1_pred_csv}")
            self.logger.info(f"   Rows      : {len(_l1_pdf)}")
            self.logger.info(f"   Correct   : {_l1_corr}/{len(_l1_pdf)} ({_l1_corr/len(_l1_pdf)*100:.2f}%)")
            self.logger.info("   Columns   : subject | message | text |")
            self.logger.info("              true_masterdepartment | predicted_masterdepartment | correct")
            self.logger.info("=" * 70)
        except Exception as _e:
            self.logger.warning(f"⚠️  Could not save level1_predictions.csv: {_e}")
    ...
    def _train_level2(self, train_df, test_df):
        ...
        # Training inputs — use ground truth prefix (correct for training)
        train_texts_conditional = [
            f"MasterDepartment: {row['masterdepartment']} Email: {row['text']}"
            for _, row in train_df.iterrows()
        ]

        # TEST inputs — use L1 MODEL PREDICTION as prefix (NOT ground truth)
        # Using GT prefix in test = teacher forcing = inflated scores
        self.logger.info("=" * 70)
        self.logger.info("🔧 L2 TEST INPUTS: using L1 model predictions (no teacher forcing)")
        self.logger.info("=" * 70)
        _l1p = os.path.join(self.model_dir, self.finetuned_model_name)
        import pickle as _pkl
        with open(os.path.join(_l1p, 'masterdepartment_encoder.pkl'), 'rb') as _fe:
            _l1enc = _pkl.load(_fe)
        _l1mod = MasterDepartmentModel(self.baseline_model_name, num_labels=len(_l1enc.classes_))
        _l1mod.load(_l1p)
        _l1mod.to(device)
        _l1mod.eval()
        self.logger.info(f"✅ L1 model loaded for L2 test prefix generation")
        test_texts_conditional = []
        test_pred_masterdept   = []
        with torch.no_grad():
            for _i, (_, _r) in enumerate(test_df.iterrows()):
                _ids, _msk = self._tokenize_and_pad([_r['text']], self.tokenizer, 256)
                _lg = _l1mod(torch.tensor(_ids).to(device), torch.tensor(_msk).to(device))
                _pmd = _l1enc.inverse_transform([torch.argmax(_lg, dim=1).cpu().numpy()[0]])[0]
                test_pred_masterdept.append(_pmd)
                _l2_input_str = f"MasterDepartment: {_pmd} Email: {_r['text']}"
                test_texts_conditional.append(_l2_input_str)
                # LOG AT INFO so it is visible in training log
                self.logger.info(
                    f"  📨 L2 TEST INPUT [{_i+1}/{len(test_df)}] "
                    f"| L1_PRED='{_pmd}' "
                    f"| GT_MD='{_r['masterdepartment']}' "
                    f"| {'✅ L1 CORRECT' if _r['masterdepartment']==_pmd else '❌ L1 WRONG'}"
                )
                self.logger.info(f"     FULL STRING SENT TO L2 MODEL: "{_l2_input_str[:120]}"")
        self.logger.info("")
        self.logger.info(f"✅ L2 TEST INPUTS BUILT: {len(test_texts_conditional)} inputs using L1 model predictions")
        self.logger.info(f"   First 3 sample inputs sent to L2 model:")
        for _si, _sinp in enumerate(test_texts_conditional[:3]):
            self.logger.info(f"   [{_si+1}] {_sinp[:150]}")
        self.logger.info("")

        # ═══════════════════════════════════════════════════════════════════
        # PROOF LOG — LEVEL 2 TRAINING INPUT
        # ═══════════════════════════════════════════════════════════════════
        self.logger.info("")
        self.logger.info("=" * 70)
        self.logger.info("📋 LEVEL 2 TRAINING — WHAT GOES INTO THE MODEL")
        self.logger.info("=" * 70)
        self.logger.info("  ✅ INPUT  : 'MasterDepartment: [GT]  Email: [text]'")
        self.logger.info("  ✅ LABEL  : 'department' column — read directly from Label Studio annotation")
        self.logger.info("  ✅ SOURCE OF MasterDepartment IN PREFIX:")
        self.logger.info("             row['masterdepartment']  ← comes from DataFrame column")
        self.logger.info("             which is the HUMAN ANNOTATION from Label Studio")
        self.logger.info("             ⚠️  The Level 1 trained model is NOT called here")
        self.logger.info("             ⚠️  Level 1 model output is NOT used during Level 2 training")
        self.logger.info(f"  ✅ LABEL COLUMN USED : df['department']")
        self.logger.info(f"  ✅ SAVE PATH : {os.path.join(self.model_dir, 'department_model')}")
        self.logger.info("")
        self.logger.info("  📌 SAMPLE INPUTS GOING INTO LEVEL 2 MODEL:")
        for idx, (_, row) in enumerate(train_df.head(3).iterrows()):
            combined = f"MasterDepartment: {row['masterdepartment']} Email: {row['text']}"
            self.logger.info(f"  ┌─ Sample {idx+1} ────────────────────────────────────────────────")
            self.logger.info(f"  │  PREFIX (GT annotation)  : \"MasterDepartment: {row['masterdepartment']}\"")
            self.logger.info(f"  │  EMAIL TEXT              : \"{str(row['text'])}\"")
            self.logger.info(f"  │  FULL INPUT TO MODEL     : \"{combined}\"")
            self.logger.info(f"  │  LABEL (GT annotation)   : \"{row['department']}\"")
            self.logger.info(f"  └──────────────────────────────────────────────────────────────")
        self.logger.info("=" * 70)
        self.logger.info("")
        # ═══════════════════════════════════════════════════════════════════
        ...
        # ── Training setup ────────────────────────────────────────────────────
        NUM_EPOCHS    = int(os.getenv('L2_TRAIN_EPOCHS', 8))
        LEARNING_RATE = float(os.getenv('LEARNING_RATE', 2e-5))

        # Class weights sqrt-softened for rare departments
        raw_weights = compute_class_weight(
            class_weight='balanced',
        ...
        raw_weights  = np.clip(raw_weights, 0.5, 5.0)
        class_weights_tensor = torch.tensor(raw_weights, dtype=torch.float32).to(device)
        self.logger.info(f"⚖️  L2 class weights (sqrt-softened) — min: {raw_weights.min():.3f}  "
                         f"max: {raw_weights.max():.3f}  classes at 5x cap: {(raw_weights >= 4.99).sum()}")
        ...
        best_epoch_num, best_macro, best_ckpt_dir = max(epoch_macro_scores, key=lambda x: x[1])
        self.logger.info("=" * 80)
        self.logger.info(f"🏆 L2 BEST EPOCH AUTO-SELECTED: Epoch {best_epoch_num} | Macro-F1: {best_macro:.4f}")
        self.logger.info(f"   Source : {best_ckpt_dir}")
        self.logger.info("   Epoch Summary:")
        for ep_num, ep_f1, _ in epoch_macro_scores:
            marker = " ← BEST" if ep_num == best_epoch_num else ""
        ...
        # ── Final evaluation using best epoch weights ──────────────────────────
        self.logger.info("📊 Evaluating Level 2 model (best epoch)...")
        model.load(save_path)
        model.to(device)
        model.eval()
        ...
        # ── Log each test email flow for Level 2 ──────────────────────────────
        self.logger.info("")
        self.logger.info("=" * 70)
        self.logger.info("🧪 LEVEL 2 — TEST SET EVALUATION (20% split) — EMAIL BY EMAIL")
        self.logger.info("   PREFIX : L1 MODEL PREDICTION (not ground truth — no teacher forcing)")
        self.logger.info("   LABEL  : department (ground truth from annotation)")
        self.logger.info("=" * 70)
        _tdf_l2 = test_df.reset_index(drop=True)
        for idx, row in _tdf_l2.iterrows():
            true_label = row['department']
            pred_label = dept_encoder.inverse_transform([dept_preds[idx]])[0]
            full_input = test_texts_conditional[idx]
            pred_md    = test_pred_masterdept[idx] if idx < len(test_pred_masterdept) else row['masterdepartment']
            match      = "✅ CORRECT" if true_label == pred_label else "❌ WRONG"
            md_ok      = "✅" if pred_md == row['masterdepartment'] else "❌ L1 WRONG"
            self.logger.info(f"  Test Email [{idx+1}]")
            self.logger.info(f"    EMAIL TEXT         : '{row['text']}'")
            self.logger.info(f"    GT MasterDept      : '{row['masterdepartment']}'")
            self.logger.info(f"    L1 PRED MasterDept : '{pred_md}'  {md_ok}")
            self.logger.info(f"    FULL INPUT SENT    : '{full_input}'")
            self.logger.info(f"    GT Department      : '{true_label}'")
            self.logger.info(f"    PREDICTED Dept     : '{pred_label}'")
            self.logger.info(f"    RESULT             : {match}")
            self.logger.info("")
        ...
        self.logger.info(f"✅ Department (best epoch {best_epoch_num}) - Accuracy: {dept_accuracy:.4f} | F1: {dept_f1:.4f}")
        ...
        # ── Save level2_predictions.csv ──────────────────────────────────────
        # Contains: email text, what was sent to L2 (L1 predicted prefix + email),
        # ground truth department, predicted department, and correct flag
        try:
            _l2_pred_csv = os.path.join(self.model_dir, 'level2_predictions.csv')
            _l2_tdf      = test_df.reset_index(drop=True)
            _l2_rows     = []
            for _li, _lr in _l2_tdf.iterrows():
                _l1_pred_md = test_pred_masterdept[_li] if _li < len(test_pred_masterdept) else _lr.get('masterdepartment', '')
                _txt        = _lr.get('text', '')
                _l2_rows.append({
                    'subject'              : _lr.get('subject', ''),
                    'message'              : _lr.get('message', _txt),
                    'text'                 : _txt,
                    'true_masterdepartment': _lr.get('masterdepartment', ''),
                    'l1_pred_masterdept'   : _l1_pred_md,
                    'l1_correct'           : 'YES' if _l1_pred_md == _lr.get('masterdepartment', '') else 'NO',
                    'l2_input_sent'        : f"MasterDepartment: {_l1_pred_md} Email: {_txt}"[:400],
                    'true_department'      : decoded_dept_true[_li],
                    'predicted_department' : decoded_dept_pred[_li],
                    'correct'              : 'YES' if decoded_dept_true[_li] == decoded_dept_pred[_li] else 'NO',
                })
            _l2_pdf  = pd.DataFrame(_l2_rows)
            _l2_corr = (_l2_pdf['correct'] == 'YES').sum()
            _l2_pdf.to_csv(_l2_pred_csv, index=False, encoding='utf-8')
            self.logger.info("=" * 70)
            self.logger.info("💾 LEVEL 2 PREDICTIONS CSV SAVED (after training)")
            self.logger.info(f"   Path      : {_l2_pred_csv}")
            self.logger.info(f"   Rows      : {len(_l2_pdf)}")
            self.logger.info(f"   Correct   : {_l2_corr}/{len(_l2_pdf)} ({_l2_corr/len(_l2_pdf)*100:.2f}%)")
            self.logger.info("   Columns   : subject | message | text |")
            self.logger.info("              true_masterdepartment | l1_pred_masterdept | l1_correct |")
            self.logger.info("              l2_input_sent  ← exact string sent to L2 model |")
            self.logger.info("              true_department | predicted_department | correct")
            self.logger.info("=" * 70)
        except Exception as _e:
            self.logger.warning(f"⚠️  Could not save level2_predictions.csv: {_e}")
    ...
    def _train_level3(self, train_df, test_df):
        ...
        # Training inputs — use ground truth prefixes (correct for training)
        train_texts_conditional = [
            f"MasterDepartment: {row['masterdepartment']} Department: {row['department']} Email: {row['text']}"
            for _, row in train_df.iterrows()
        ]

        # TEST inputs — use L1+L2 MODEL PREDICTIONS (NOT ground truth)
        # Using GT prefixes in test = teacher forcing = inflated / dishonest accuracy
        self.logger.info("=" * 70)
        self.logger.info("🔧 L3 TEST INPUTS: using L1+L2 model predictions (no teacher forcing)")
        self.logger.info("=" * 70)
        _l1p3 = os.path.join(self.model_dir, self.finetuned_model_name)
        _l2p3 = os.path.join(self.model_dir, 'department_model')
        import pickle as _pkl3
        with open(os.path.join(_l1p3, 'masterdepartment_encoder.pkl'), 'rb') as _f3a:
            _l1enc3 = _pkl3.load(_f3a)
        with open(os.path.join(_l2p3, 'department_encoder.pkl'), 'rb') as _f3b:
            _l2enc3 = _pkl3.load(_f3b)
        _l1m3 = MasterDepartmentModel(self.baseline_model_name, num_labels=len(_l1enc3.classes_))
        _l1m3.load(_l1p3); _l1m3.to(device); _l1m3.eval()
        _l2m3 = DepartmentNNModel(self.baseline_model_name, num_labels=len(_l2enc3.classes_))
        _l2m3.load(_l2p3); _l2m3.to(device); _l2m3.eval()
        self.logger.info(f"✅ L1 model loaded for L3 test prefix: {_l1p3}")
        self.logger.info(f"✅ L2 model loaded for L3 test prefix: {_l2p3}")
        test_texts_conditional  = []
        test_pred_masterdept_l3 = []
        test_pred_dept_l3       = []
        with torch.no_grad():
            for _i3, (_, _r3) in enumerate(test_df.iterrows()):
                # ── Step 1: Run L1 to get MasterDepartment prediction ───────────
                _ids1, _msk1 = self._tokenize_and_pad([_r3['text']], self.tokenizer, 256)
                _lg1  = _l1m3(torch.tensor(_ids1).to(device), torch.tensor(_msk1).to(device))
                _pmd3 = _l1enc3.inverse_transform([torch.argmax(_lg1, dim=1).cpu().numpy()[0]])[0]

                # ── Step 2: Run L2 using L1 prediction as prefix ────────────────
                _l2in = f"MasterDepartment: {_pmd3} Email: {_r3['text']}"
                _ids2, _msk2 = self._tokenize_and_pad([_l2in], self.tokenizer, 256)
                _lg2  = _l2m3(torch.tensor(_ids2).to(device), torch.tensor(_msk2).to(device))
                _pd3  = _l2enc3.inverse_transform([torch.argmax(_lg2, dim=1).cpu().numpy()[0]])[0]

                # ── Step 3: Build L3 input using BOTH predictions ───────────────
                _l3in = f"MasterDepartment: {_pmd3} Department: {_pd3} Email: {_r3['text']}"

                test_pred_masterdept_l3.append(_pmd3)
                test_pred_dept_l3.append(_pd3)
                test_texts_conditional.append(_l3in)

                # LOG AT INFO so it is visible in training log
                self.logger.info(
                    f"  📨 L3 TEST INPUT [{_i3+1}/{len(test_df)}] "
                    f"| L1_PRED_MD='{_pmd3}' {'✅' if _r3['masterdepartment']==_pmd3 else '❌'} "
                    f"| L2_PRED_D='{_pd3}' {'✅' if _r3['department']==_pd3 else '❌'}"
                )
                self.logger.info(f"     L1 input sent  : "{_r3['text'][:80]}"")
                self.logger.info(f"     L2 input sent  : "{_l2in[:120]}"")
                self.logger.info(f"     L3 input sent  : "{_l3in[:150]}"")
        self.logger.info("")
        self.logger.info(f"✅ L3 TEST INPUTS BUILT: {len(test_texts_conditional)} inputs using L1+L2 model predictions")
        self.logger.info(f"   First 3 sample inputs sent to L3 model:")
        for _si, _sinp in enumerate(test_texts_conditional[:3]):
            self.logger.info(f"   [{_si+1}] {_sinp[:180]}")
        self.logger.info("")

        # ═══════════════════════════════════════════════════════════════════
        # PROOF LOG — LEVEL 3 TRAINING INPUT
        # ═══════════════════════════════════════════════════════════════════
        self.logger.info("")
        self.logger.info("=" * 70)
        self.logger.info("📋 LEVEL 3 TRAINING — WHAT GOES INTO THE MODEL")
        self.logger.info("=" * 70)
        self.logger.info("  ✅ INPUT  : 'MasterDepartment: [GT]  Department: [GT]  Email: [text]'")
        self.logger.info("  ✅ LABEL  : 'querytype' column — read directly from Label Studio annotation")
        self.logger.info("  ✅ SOURCE OF MasterDepartment IN PREFIX:")
        self.logger.info("             row['masterdepartment']  ← HUMAN ANNOTATION from Label Studio")
        self.logger.info("             ⚠️  Level 1 model is NOT called here")
        self.logger.info("  ✅ SOURCE OF Department IN PREFIX:")
        self.logger.info("             row['department']  ← HUMAN ANNOTATION from Label Studio")
        self.logger.info("             ⚠️  Level 2 model is NOT called here")
        self.logger.info("  ✅ BOTH PREFIXES come from ground truth — no model output used")
        self.logger.info(f"  ✅ LABEL COLUMN USED : df['querytype']")
        self.logger.info(f"  ✅ SAVE PATH : {os.path.join(self.model_dir, 'querytype_model')}")
        self.logger.info("")
        self.logger.info("  📌 SAMPLE INPUTS GOING INTO LEVEL 3 MODEL:")
        for idx, (_, row) in enumerate(train_df.head(3).iterrows()):
            ...
            self.logger.info(f"  │  EMAIL TEXT              : \"{str(row['text'])}\"")
            self.logger.info(f"  │  FULL INPUT TO MODEL     : \"{combined}\"")
            self.logger.info(f"  │  LABEL (GT annotation)   : \"{row['querytype']}\"")
            self.logger.info(f"  └──────────────────────────────────────────────────────────────")
        self.logger.info("=" * 70)
        self.logger.info("")
        # ═══════════════════════════════════════════════════════════════════
        ...
        # ── Training setup ────────────────────────────────────────────────────
        NUM_EPOCHS    = int(os.getenv('L3_TRAIN_EPOCHS', 10))
        LEARNING_RATE = float(os.getenv('LEARNING_RATE', 2e-5))

        # Class weights sqrt-softened — 148 classes, 62 with <10 samples
        raw_weights = compute_class_weight(
            class_weight='balanced',
        ...
        raw_weights  = np.clip(raw_weights, 0.5, 5.0)
        class_weights_tensor = torch.tensor(raw_weights, dtype=torch.float32).to(device)
        self.logger.info(f"⚖️  L3 class weights (sqrt-softened) — min: {raw_weights.min():.3f}  "
                         f"max: {raw_weights.max():.3f}  classes at 5x cap: {(raw_weights >= 4.99).sum()}")
        ...
        best_epoch_num, best_macro, best_ckpt_dir = max(epoch_macro_scores, key=lambda x: x[1])
        self.logger.info("=" * 80)
        self.logger.info(f"🏆 L3 BEST EPOCH AUTO-SELECTED: Epoch {best_epoch_num} | Macro-F1: {best_macro:.4f}")
        self.logger.info(f"   Source : {best_ckpt_dir}")
        self.logger.info("   Epoch Summary:")
        for ep_num, ep_f1, _ in epoch_macro_scores:
            marker = " ← BEST" if ep_num == best_epoch_num else ""
        ...
        # ── Final evaluation using best epoch weights ──────────────────────────
        self.logger.info("📊 Evaluating Level 3 model (best epoch)...")
        model.load(save_path)
        model.to(device)
        model.eval()
        ...
        # ── Log each test email flow for Level 3 ──────────────────────────────
        self.logger.info("")
        self.logger.info("=" * 70)
        self.logger.info("🧪 LEVEL 3 — TEST SET EVALUATION (20% split) — EMAIL BY EMAIL")
        self.logger.info("   PREFIX-1 : L1 MODEL PREDICTION (not ground truth)")
        self.logger.info("   PREFIX-2 : L2 MODEL PREDICTION (not ground truth)")
        self.logger.info("   Gives honest real-world accuracy — no teacher forcing")
        self.logger.info("=" * 70)
        _tdf_l3 = test_df.reset_index(drop=True)
        for idx, row in _tdf_l3.iterrows():
            true_label = row['querytype']
            pred_label = qt_encoder.inverse_transform([qt_preds[idx]])[0]
            full_input = test_texts_conditional[idx]
            pred_md    = test_pred_masterdept_l3[idx] if idx < len(test_pred_masterdept_l3) else row['masterdepartment']
            pred_d     = test_pred_dept_l3[idx]       if idx < len(test_pred_dept_l3)       else row['department']
            match      = "✅ CORRECT" if true_label == pred_label else "❌ WRONG"
            md_ok      = "✅" if pred_md == row['masterdepartment'] else "❌ L1 WRONG"
            d_ok       = "✅" if pred_d  == row['department']       else "❌ L2 WRONG"
            self.logger.info(f"  Test Email [{idx+1}]")
            self.logger.info(f"    EMAIL TEXT          : '{row['text']}'")
            self.logger.info(f"    GT MasterDept       : '{row['masterdepartment']}'")
            self.logger.info(f"    L1 PRED MasterDept  : '{pred_md}'  {md_ok}")
            self.logger.info(f"    GT Department       : '{row['department']}'")
            self.logger.info(f"    L2 PRED Department  : '{pred_d}'   {d_ok}")
            self.logger.info(f"    FULL INPUT SENT     : '{full_input}'")
            self.logger.info(f"    GT QueryType        : '{true_label}'")
            self.logger.info(f"    PREDICTED QueryType : '{pred_label}'")
            self.logger.info(f"    RESULT              : {match}")
            self.logger.info("")
        ...
        self.logger.info(f"✅ QueryType (best epoch {best_epoch_num}) - Accuracy: {qt_accuracy:.4f} | F1: {qt_f1:.4f}")
        ...
        # Save level3_predictions.csv with subject/message/GT/predicted labels
        try:
            _pcsv = os.path.join(self.model_dir, 'level3_predictions.csv')
            _tdf2 = test_df.reset_index(drop=True)
            _rows = []
            for _pi, _pr in _tdf2.iterrows():
                _l1pred  = test_pred_masterdept_l3[_pi] if _pi < len(test_pred_masterdept_l3) else ''
                _l2pred  = test_pred_dept_l3[_pi]       if _pi < len(test_pred_dept_l3)       else ''
                _txt     = _pr.get('text', '')
                _rows.append({
                    'subject'               : _pr.get('subject', ''),
                    'message'               : _pr.get('message', _txt),
                    'text'                  : _txt,
                    'true_masterdepartment' : _pr.get('masterdepartment', ''),
                    'true_department'       : _pr.get('department', ''),
                    'true_querytype'        : decoded_qt_true[_pi],
                    'l1_pred_masterdept'    : _l1pred,
                    'l1_correct'            : 'YES' if _l1pred == _pr.get('masterdepartment','') else 'NO',
                    'l2_pred_department'    : _l2pred,
                    'l2_correct'            : 'YES' if _l2pred == _pr.get('department','') else 'NO',
                    'l2_input_sent'         : f"MasterDepartment: {_l1pred} Email: {_txt}"[:300],
                    'l3_input_sent'         : f"MasterDepartment: {_l1pred} Department: {_l2pred} Email: {_txt}"[:300],
                    'predicted_querytype'   : decoded_qt_pred[_pi],
                    'correct'               : 'YES' if decoded_qt_true[_pi] == decoded_qt_pred[_pi] else 'NO',
                })
            _pdf = pd.DataFrame(_rows)
            _corr = (_pdf['correct'] == 'YES').sum()
            _pdf.to_csv(_pcsv, index=False, encoding='utf-8')
            self.logger.info("=" * 70)
            self.logger.info("💾 LEVEL 3 PREDICTIONS CSV SAVED (after training)")
            self.logger.info(f"   Path      : {_pcsv}")
            self.logger.info(f"   Rows      : {len(_pdf)}")
            self.logger.info(f"   Correct   : {_corr}/{len(_pdf)} ({_corr/len(_pdf)*100:.2f}%)")
            self.logger.info("   Columns   : subject | message | text")
            self.logger.info("              | true_masterdepartment | true_department | true_querytype")
            self.logger.info("              | l1_pred_masterdept | l1_correct")
            self.logger.info("              | l2_pred_department | l2_correct")
            self.logger.info("              | l2_input_sent  ← exact string sent to L2 model")
            self.logger.info("              | l3_input_sent  ← exact string sent to L3 model")
            self.logger.info("              | predicted_querytype | correct")
            self.logger.info("=" * 70)
        except Exception as e:
            self.logger.warning(f"Could not save level3_predictions.csv: {e}")
                ...
                with torch.no_grad():
                    for batch_idx, batch in enumerate(dataloader):
                        ...
                        try:
                            self.logger.info("")
                            self.logger.info("=" * 70)
                            self.logger.info("🔍 INFERENCE — LEVEL 1 : MasterDepartmentModel")
                            self.logger.info("=" * 70)
                            self.logger.info("  INPUT : Raw email text only — no prefix")
                            ...
                            for idx_l1, pred_l1 in enumerate(decoded_masterdept_preds):
                                ...
                                self.logger.info(f"    Email[{idx_l1}] → MasterDepartment = '{pred_l1}'  (score: {score_l1:.4f})")
                            self.logger.info("=" * 70)
                        ...
                        for i in range(len(masterdept_preds)):
                            try:
                                ...
                                self.logger.info("")
                                self.logger.info("=" * 70)
                                self.logger.info(f"📧 EMAIL [{global_text_idx}] — FULL RAW TEXT RECEIVED FOR PREDICTION:")
                                self.logger.info(f"    {current_text}")
                                self.logger.info(f"  L1 RESULT → MasterDepartment = '{decoded_masterdept_preds[i]}'  (score: {float(masterdept_probs[i][masterdept_preds[i]].item()):.4f})")
                                self.logger.info("=" * 70)
                                ...
                                if self.department_model is not None:
                                    try:
                                        ...
                                        self.logger.info("")
                                        self.logger.info("=" * 70)
                                        self.logger.info("🔍 INFERENCE — LEVEL 2 : DepartmentNNModel")
                                        self.logger.info("=" * 70)
                                        self.logger.info("  HOW INPUT IS BUILT:")
                                        self.logger.info(f"    PREFIX  → 'MasterDepartment: {decoded_masterdept_preds[i]}'")
                                        self.logger.info(f"              ↑ this comes from Level 1 model output (NOT annotation)")
                                        self.logger.info(f"    EMAIL   → '{current_text}'")
                                        ...
                                        if original_prediction_idx != dept_idx:
                                            self.logger.warning(f"  ⚠️  HIERARCHICAL CONSTRAINT changed prediction!")
                                            ...
                                            self.logger.info(f"  ✅ LEVEL 2 RESULT : '{department_label}'  (score: {final_score:.3f})")
                                        ...
                                        if self.querytype_model is not None:
                                            try:
                                                ...
                                                self.logger.info("")
                                                self.logger.info("=" * 70)
                                                self.logger.info("🔍 INFERENCE — LEVEL 3 : QueryTypeNNModel")
                                                self.logger.info("=" * 70)
                                                self.logger.info("  HOW INPUT IS BUILT:")
                                                self.logger.info(f"    PREFIX-1 → 'MasterDepartment: {decoded_masterdept_preds[i]}'")
                                                self.logger.info(f"               ↑ from Level 1 model output")
                                                self.logger.info(f"    PREFIX-2 → 'Department: {department_label}'")
                                                self.logger.info(f"               ↑ from Level 2 model output")
                                                self.logger.info(f"    EMAIL    → '{current_text}'")
                                                self.logger.info(f"  FULL STRING SENT TO MODEL:")
                                                ...
                                                if original_prediction_idx != qt_idx:
                                                    ...
                                                    self.logger.info(f"  ✅ LEVEL 3 RESULT : '{querytype_label}'  (score: {final_score:.3f})")
                                                ...
                                                self.logger.info("")
                                                self.logger.info("╔══════════════════════════════════════════════════════════════╗")
                                                self.logger.info("║  FINAL PREDICTION SUMMARY")
                                                self.logger.info("╠══════════════════════════════════════════════════════════════╣")
                                                self.logger.info(f"║  L1 MasterDept  : '{decoded_masterdept_preds[i]}'")
                                                self.logger.info(f"║  L2 Department  : '{department_label}'")
                                                self.logger.info(f"║  L3 QueryType   : '{querytype_label}'")
                                                self.logger.info("╚══════════════════════════════════════════════════════════════╝")
