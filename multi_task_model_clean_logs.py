...
class MultiTaskBertModel:
    ...
    def _train_level1(self, train_df, test_df):
        ...
        for idx, (_, row) in enumerate(train_df.head(3).iterrows()):
            ...
            self.logger.info(f"  │  INPUT TEXT  : \"{str(row['text'])[:120]}\"")
    ...
    def _train_level2(self, train_df, test_df):
        ...
        for idx, (_, row) in enumerate(train_df.head(3).iterrows()):
            ...
            self.logger.info(f"  │  EMAIL TEXT              : \"{str(row['text'])[:80]}\"")
            self.logger.info(f"  │  FULL INPUT TO MODEL     : \"{combined[:150]}\"")
                            ...
                            try:
                                ...
                                if self.department_model is not None:
                                    try:
                                        ...
                                        self.logger.info(f"    EMAIL   → '{current_text[:100]}{'...' if len(current_text)>100 else ''}'")
                                        self.logger.info(f"  FULL STRING SENT TO MODEL:")
                                        self.logger.info(f"    >>> '{conditional_text[:200]}{'...' if len(conditional_text)>200 else ''}'")
                                        self.logger.info("=" * 70)
                                        ...
                                        if self.querytype_model is not None:
                                            try:
                                                ...
                                                self.logger.info(f"    EMAIL    → '{current_text[:100]}{'...' if len(current_text)>100 else ''}'")
                                                self.logger.info(f"  FULL STRING SENT TO MODEL:")
                                                self.logger.info(f"    >>> '{conditional_text_qt[:250]}{'...' if len(conditional_text_qt)>250 else ''}'")
                                                self.logger.info("=" * 70)
