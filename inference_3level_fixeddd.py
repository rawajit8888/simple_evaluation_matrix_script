...
def main():
    ...
    # ===== SMOKE TEST: try one row before running full batch =====
    print(f"\n🧪 Smoke test — sending 1 row to verify endpoint is reachable...")
    ...
    try:
        ...
        if _test_resp.status_code != 200:
            print(f"\n❌ Smoke test failed — got HTTP {_test_resp.status_code} from {PREDICT_URL_BERT}")
            print("   Check that your model server is running and PREDICT_URL_BERT is correct.")
            return
        else:
            print("✓ Smoke test passed — endpoint is reachable")
    except Exception as e:
        print(f"\n❌ Smoke test connection error: {e}")
        print(f"   Could not reach {PREDICT_URL_BERT} — is the server running?")
        return
    ...
    if TRUE_MASTERDEPT_COL in df.columns and TRUE_DEPT_COL in df.columns and TRUE_QUERYTYPE_COL in df.columns:
        ...
        if len(eval_df) == 0:
            print("\n❌ No valid predictions to evaluate — all rows errored.")
            print("   Check that the model server is running and returning valid responses.")
            print("\n✅ INFERENCE COMPLETE (with errors)")
            return
