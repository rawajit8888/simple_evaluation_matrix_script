    ...
    def _train_level2(self, train_df, test_df):
        ...
        with torch.no_grad():
            for _i, (_, _r) in enumerate(test_df.iterrows()):
                ...
                test_texts_conditional.append(f"MasterDepartment: {_pmd} Email: {_r['text']}")
                self.logger.debug(
                    f"  L2 test[{_i+1}] GT='{_r['masterdepartment']}' PRED='{_pmd}' "
                    f"{'OK' if _r['masterdepartment']==_pmd else 'WRONG'}"
                )
        self.logger.info(f"✅ Built {len(test_texts_conditional)} L2 test inputs using L1 predictions")
    ...
    def _train_level3(self, train_df, test_df):
        ...
        with torch.no_grad():
            for _i3, (_, _r3) in enumerate(test_df.iterrows()):
                ...
                test_texts_conditional.append(
                    f"MasterDepartment: {_pmd3} Department: {_pd3} Email: {_r3['text']}"
                )
                self.logger.debug(
                    f"  L3 test[{_i3+1}]: GT_MD='{_r3['masterdepartment']}' PRED_MD='{_pmd3}' "
                    f"{'OK' if _r3['masterdepartment']==_pmd3 else 'WRONG'} | "
                    f"GT_D='{_r3['department']}' PRED_D='{_pd3}' "
                    f"{'OK' if _r3['department']==_pd3 else 'WRONG'}"
                )
        self.logger.info(f"✅ Built {len(test_texts_conditional)} L3 test inputs using L1+L2 predictions")
        ...
        try:
            ...
            for _pi, _pr in _tdf2.iterrows():
                _rows.append({
                    ...
                    'message'               : _pr.get('message', _pr.get('text', '')),
                    'text'                  : _pr.get('text', ''),
                    'true_masterdepartment' : _pr.get('masterdepartment', ''),
                    'true_department'       : _pr.get('department', ''),
                    'true_querytype'        : decoded_qt_true[_pi],
                    'l1_pred_masterdept'    : test_pred_masterdept_l3[_pi] if _pi < len(test_pred_masterdept_l3) else '',
                    'l2_pred_department'    : test_pred_dept_l3[_pi]       if _pi < len(test_pred_dept_l3)       else '',
            ...
            self.logger.info("   Columns   : subject | message | text | true_masterdepartment |")
            self.logger.info("              true_department | true_querytype | l1_pred_masterdept |")
            self.logger.info("              l2_pred_department | predicted_querytype | correct")
            self.logger.info("=" * 70)
        except Exception as e:
